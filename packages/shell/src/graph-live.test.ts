...
import { buildFixture, OUTSIDE_WORK, type Fixture } from './fixture'
import { liveEngine, read } from './live'

/**
 * The graph, read from a fixture that holds every edge still.
 *
 * **Every edge a finished backlog loses** (RG196, RG219). This file used to hunt the repository
 * for a line blocked by another open one, a line whose shipping frees something, and a line
 * waiting on work outside the backlog — calling that edge permanent, since no ship here clears
 * one. Draining the backlog removed the first two and reddened three assertions (RG196); a
 * backlog whose last outside dep resolved removed the third and reddened two more (RG219), on
 * days the reader had not changed. A fixture with one chained line and one waiting outside holds
 * all three, and no work in this repository can take one away.
 */

async function graphOf(id: string, root: string): Promise<Graph> {
...
beforeAll(async () => {
  // Two open lines, the second waiting on the first: one blocker, one cascade. And a third
  // waiting on work outside the backlog. None is anything a ship in this repository can take
  // away.
  fixture = await buildFixture(liveEngine, {
    open: 2,
    shipped: 1,
    deferred: 0,
    chained: true,
  ...
  for (const id of listedTasks(await read(fixture.root, 'list', {})).map((one) => one.id)) {
    ...
    if (blocked && leverage && outside) break
  }
}, 300000)
...
describe('RG25: a blocker inside the backlog against one outside it', () => {
  it('finds a line waiting on work no ship here can ever satisfy', () => {
    ...
    expect(outside?.outside[0]?.dep).toBe(OUTSIDE_WORK)
