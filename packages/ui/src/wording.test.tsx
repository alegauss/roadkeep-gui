...
import { atSurface, everyRoute, SURFACE_DATA } from './surface-harness'
 ...
 *
 * The fixture is `surface-harness`' since RG214, which asks the same question of the same eight
 * surfaces in a real browser. `SURFACE_DATA` is the prose that fixture feeds through — a
 * symptom, an id, a path — because what this run is about is the strings this app typed, not
 * the ones a project wrote.
 */
...
describe('RG176: every surface a reader can route to, under the pseudo-locale', () => {
  it('wraps every sentence on each of them, leaving only what the project wrote bare', async () => {
    const bareHere = (found: readonly string[]) => found.filter((text) => !SURFACE_DATA.has(text))

    for (const route of everyRoute()) {
      const drawn = await atSurface(route, { over: pseudo() })
