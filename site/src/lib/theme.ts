...
// reading the effective theme and flipping it. The tokens the body background reads are keyed
// off the data-theme attribute, so setting the attribute is the whole act.
//
// The app makes the same promise: its window follows the OS ground until a person picks one,
// and a site that disagreed with its own product about that would be saying so in public.

export type Theme = 'light' | 'dark'

const STORAGE_KEY = 'rkg-theme'
