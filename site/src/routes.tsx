...
export type RouteMeta = {
  /** app path, leading slash: "/" or "/compare" or "/features/portfolio" */
  path: string
  title: string
  description: string
  ogTitle: string
...
export const ROUTE_META: RouteMeta[] = [
  ...
  // The depth pages' metadata is read off the same records as their routes and pages below, so
  // a page cannot ship with a route and no title, or a title and no route.
  ...features.map((f) => ({
...
// single source, so a route cannot render one page on the client and another in the static file.
...
// metadata but no component never gets a file. Both are silent at runtime, so they are made loud
// at import: this throw fails the build or the prerender, whichever imports first.
