...
for (const meta of ROUTE_META) {
  ...
  console.log(
    `prerendered ${meta.path.padEnd(24)} -> ${htmlPath} + ${mdPath}  (${byteLength(markdown)} md bytes)`,
  )
}

// The manifest lists the routes, their twins and their sizes, so an agent sent to evaluate this
// app finds every page as a flat file without running a line of JavaScript. No build timestamp:
...
// a deleted one disappears with no second list to keep current.
 ...
 * page did. One date for every route rather than one per route, because a per-URL date would
 * need to know which sources compose which page, and that is a second list this generator
 * exists to avoid.
 */
...
const urls = ROUTE_META.map((meta) => {
  ...
  // No changefreq and no priority: the crawlers that read this file say outright that they
  // ignore both, and a hint nobody reads is a claim nobody checks.
  return lastmod
