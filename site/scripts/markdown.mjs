...
// the *composition* lives in the JSX, so the twin is produced from the rendered HTML, never
// authored a second time from the data, which would declare the composition again and let the
// two drift. The nav and footer never reach a twin (dropped by tag) and a call to action is
// dropped by its data-twin="omit" attribute.
...
// numbers, the section kicker, terminal window bars, the legend.
const SKIP_CLASSES = new Set(['dot', 'chk', 'ico', 'n', 'eyebrow', 'bar', 'legend', 'copy-btn'])
...
// content as one raw text node, colour spans and all, so the fence is cleaned by stripping the
// tags out of this and only then decoding entities: an escaped `&lt;` must survive the strip.
...
// A GitHub-flavoured markdown table, so the twin of a matrix is machine-readable. A row with a
// single spanning cell (a group header) becomes a bold label row.
...
function blocks(node, out) {
  for (const child of node.childNodes) {
    ...
    if (cls.includes('term')) {
      ...
      // an inline element sitting at block level (a pill, a hero-meta item) is one line, not a
      // tree to recurse and shatter into a paragraph per text node
      const t = inlineTrim(child)
      if (t) out.push(t)
    } else {
      blocks(child, out)
